...
class PeUFSpider(BaseGazetteSpider):

    name = "pe_uf"
    TERRITORY_ID = "26"
    start_date = dt.date(2022, 8, 1)
    allowed_domains = ["diariooficial.cepe.com.br"]
    BASE_URL = "https://diariooficial.cepe.com.br/diariooficialweb/#/home?diario=MQ%3D%3D"
    start_urls = [BASE_URL]

    def parse(self, response):
        for date in rrule(freq=DAILY, dtstart=self.start_date, until=self.end_date):
            date_str = date.strftime('%Y-%m-%d')
            aws_url = 'https://cepebr-prod.s3.sa-east-1.amazonaws.com'
            file_url = f'{aws_url}/1/arquivos/resumoDiario/{date_str}/{date_str}.pdf'
            yield Gazette(
                date=date.date(),
                file_urls=[file_url],
